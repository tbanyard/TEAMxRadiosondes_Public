...
Usage:
    python uvprofile_vJEMS.py -f <file_or_directory> [options]

Example:
    python src/TEAMxRadiosondes/uvprofile_vJEMS.py -f data/202502261702R5040846_Nph_all.nc -l -b red : 1.5
...
def plot_uv_alt_single_sonde(f, args):
    ...
    ax.set_title(
        ...
        f"{t:%d %b %Y %H:%M UTC}\n"
        f"{ds.attrs['site_name']}: {ds.attrs['serial']}"
    )
    ...
    leg = ax.legend(
        ...
        borderaxespad=-0.1,
        borderpad=0.4,
        handletextpad=0,   # reduce marker–text gap (default is 0.8)
        labelspacing=0.1     # tighten vertical spacing if desired
    )
    ...
    width, height = bb_axes.width-0.015, bb_axes.height-0.02
    ...
    # Font sizes
    title_fs = 16   # for "65 kts"
    mid_fs   = 11   # for "50 10 5"
    ...
    # Text labels
    ax_leg.text(x0 - 0.115, y0 - 0.11, "50 10 5", fontsize=mid_fs)
    ax_leg.text(x0 + 0.01, y0 + 0.15, "65 kts", ha="center", va="top", fontsize=title_fs)
    if args.legend_title:
        ax_leg.text(x0 - 0.027, y0 + 0.38, "Barb Legend",
    ...
    # Save figure
    figname = f"uv_profile_{timestamp}_{ds.attrs['serial']}.png"
